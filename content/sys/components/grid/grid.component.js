...
//        components:[],
  ...
  data: function(){
    return {
      ...
      mouseWheel_scroll: 3,
      cnt: 0,
      empty_mess: _e('Empty', this.class),
      sel_def: _e('All', this.class),
      agg_event: '',
      tableHeight: '',
      scrollHeight: '',
      scrollShow: false,
  ...
  oninit: function(){
    ...
    this.on({
      ...
      keypressed: function(event){ // up and down does not work in div element :(
        if(Ractive.DEBUG) console.log('keypressed fire!');
        event.original.preventDefault();
  ...
  formatters: function(){
    ...
    return {
      date: function(val, conf, col, cols, row){
        if('undefined' === typeof val || !Ractive.helper.isDate(val) || -1 != val.indexOf('0000-00-00') || (10 >= val.length && -1 != val.indexOf('00:00:00')))
          val = '';
      ...
      persent: function(val, conf, col, cols, row){
        if ('undefined' === typeof val) {
          return 'undefined';
        }
        if (row.sum_arbitration && parseInt(row.sum_arbitration, 10) === 1 ) {
          return val + "%D";
        }
        return val + "%";
      },
 ...
 *                                       opts: {all: _e('All accounts', this.class)}, // for first use - data in select
